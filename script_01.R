...
# 1. Importando o arquivo e salvando em rds. ------------------------------------------------

read_excel("data-raw/excel/grafico_cascata_rmarkdown.xlsx") |> 
  arrange(caixa_SEI, indice) |>
  saveRDS("data-raw/rds/grafico_cascata_proc_sei.rds")

# 2. Criando a função adpt_waterfall, uma adaptação da waterfalls. --------------------------------

# O objetivo é fazer algumas melhorias na waterfall original.
...
# Assim nasceu a função adpt_waterfall.

# A função waterfall original está em...
# https://github.com/HughParsonage/waterfalls/blob/master/R/waterfall.R


adpt_waterfall <- function(.data = NULL,
                           values, labels,
...
# 2. Usando a função 'adpt_waterfall' na 'faz_grafico_cascata_processos'.--------
...
# 3. Criando os df para fazer o texto sob os gráficos.------------------------------

# Cada vez que atualizar os dados é preciso fazer o df prepara_frase.

# Primeiro extraímos o valor do estoque inicial.
...
# Salvando em rds.
saveRDS(prepara_frase, "data-raw/rds/prepara_frase.rds")


# 2.2 Criando a função escrev_frase ---------------------------------------

escreve_frase <- function(audr) {
  prepara_frase |> 
    filter(caixa_sei == audr) |> 
    pull(a_frase)
}
...
# Exemplo.
escreve_frase(audr = "DAC")
escreve_frase("64.09")

# 3. Fazendo um gráfico juntando todos setores. ---------------------------
...
todas_regio |>
  ...
  waterfall(calc_total = TRUE,
            ...
            total_rect_text_color = "black") +
  labs(title = "Estoque, entradas e saídas de processos de todas caixas juntas",
       caption = "fonte: SEI-RJ/Estatísticas") +
